...
const ACTIVE_JOB_KEY = 'conviveShoppingAssistantJob';
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  void (async () => {
    ...
    if (message?.type === 'COMPLETE_CART_RESET') {
      // Dos content scripts pueden terminar el mismo vaciado durante una
      // navegación rápida. El segundo acuse debe ser idempotente: pausar aquí
      // deja el carro vacío y bloquea una carga que ya estaba autorizada.
      if (job.replaceCart && job.currentIndex === 0 && job.cartResetStatus === 'completed') {
        sendResponse({
          ok: true,
          progress: progress(job, 'Carro anterior verificado. Cargando la lista nueva…', 'loading'),
        });
        return;
      }
