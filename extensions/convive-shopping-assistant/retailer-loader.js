(() => {
  ...
  function cartControlCandidates(config) {
    ...
    const offer = (candidate, score) => {
      ...
      if (element.closest('#convive-shopping-assistant')) return;
  ...
  function blockingOverlay() {
    ...
    return [...document.querySelectorAll(
      ...
      if (!isVisible(element) || element.closest('#convive-shopping-assistant')) return false;
  ...
  function isClickableAddControl(element) {
    ...
    if (element.closest('#convive-shopping-assistant')) return false;
  ...
  function createOverlay(store) {
    const existing = document.getElementById('convive-shopping-assistant');
    if (existing) return existing;
    const overlay = document.createElement('aside');
    overlay.id = 'convive-shopping-assistant';
    overlay.innerHTML = `
  ...
  function hasVisibleEmptyCartState() {
    ...
    )].filter(element => isVisible(element) && !element.closest('#convive-shopping-assistant'));
  ...
  function findEmptyCartControl(config) {
    ...
    return [...document.querySelectorAll('button,[role="button"],a[href]')]
      .filter(element => isVisible(element) && isEnabled(element) && !element.closest('#convive-shopping-assistant'))
  ...
  void run().catch(async error => {
    const stored = await chrome.storage.local.get('conviveShoppingAssistantJob');
    const store = stored.conviveShoppingAssistantJob?.store || 'Supermercado';
