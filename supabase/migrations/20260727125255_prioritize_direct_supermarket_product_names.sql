...
AS $$
  ...
  matches AS (
    ...
    ORDER BY query.requested_term, product.id
  ),
  ranked AS (
    SELECT
      matches.*,
      ROW_NUMBER() OVER (
        ...
        ORDER BY
          ...
          END,
          CASE
            WHEN LOWER(name) LIKE LOWER(primary_anchor) || '%' THEN 0
            WHEN LOWER(name) LIKE 'bolsa ' || LOWER(primary_anchor) || '%' THEN 0
            WHEN LOWER(name) LIKE 'malla ' || LOWER(primary_anchor) || '%' THEN 0
            WHEN LOWER(name) LIKE 'pack ' || LOWER(primary_anchor) || '%' THEN 0
            ELSE 1
          END,
          ts_rank_cd(
