  ...
  ranked AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        ...
        ORDER BY
          CASE
            WHEN primary_anchor = 'leche' THEN
              CASE
                WHEN LOWER(name) !~
                  '(avena|almendra|soya|coco|vegetal|manzana|frutilla|chocolate|platano|plátano|vainilla|sabor|bebida lactea|bebida láctea|cultivada|condensada|evaporada|polvo)'
                 AND LOWER(name) ~
                  '(^|[^0-9])(900[[:space:]]*(ml|cc)|1([,.]0)?[[:space:]]*(l|lt))([^a-z]|$)'
                  THEN 0
                ELSE 1
              END
            WHEN primary_anchor = 'pechuga'
             AND LOWER(requested_term) LIKE '%pollo%' THEN
              CASE
                WHEN LOWER(name) LIKE '%pechuga%pollo%'
                 AND LOWER(name) !~ '(pavo|apanad|asada|acaramelad|cocida|fiambre|jamon|jamón|rebozad)'
                 AND (
                   LOWER(name) ~ '(^|[^0-9])([5-9][0-9]{2}|[1-9][0-9]{3,})[[:space:]]*(g|gr)([^a-z]|$)'
                   OR LOWER(name) ~ '(^|[^0-9])[01]([,.][0-9]+)?[[:space:]]*kg([^a-z]|$)'
                   OR LOWER(name) ~ '(granel|entera|deshuesada)'
                 )
                  THEN 0
                ELSE 1
              END
