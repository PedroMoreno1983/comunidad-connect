...
const CHART_COLORS = ['#8b5cf6', '#3b82f6', '#10b981', '#f43f5e', '#0ea5e9', '#d946ef', '#f59e0b'];
  ...
  return (
    <div className="rp-container">
      ...
      <PageHeader
        ...
        title="Reportes"
        subtitle={`Análisis interactivo · ${filtered.length} menciones · últimos ${dateRange} días`}
        badgeColor="orange"
        action={
        ...
        <div className="rp-chart-area">
          ...
          {chartView === 'overview' && (
            <div className="rp-overview-grid">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={topKeywords} margin={{ top: 4, right: 8, left: -22, bottom: 0 }}>
                    ...
                    <Bar dataKey="count" name="Menciones" radius={[4, 4, 0, 0]}>
                      {topKeywords.map((_, i) => <Cell key={i} fill={`url(#bar-grad-${i % CHART_COLORS.length})`} stroke={CHART_COLORS[i % CHART_COLORS.length]} strokeWidth={1} filter="url(#glow)" />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={sentimentData} cx="50%" cy="50%" innerRadius={48} outerRadius={75}
                      dataKey="value" paddingAngle={3} stroke="#ffffff" strokeWidth={2}>
                      {sentimentData.map((s, i) => <Cell key={i} fill={s.color} filter="url(#glow)" />)}
                    </Pie>
                    <Tooltip />
              ...
              {/* Fila 2 col 1 — Análisis Multidimensional (Radar) */}
              <div className="rp-chart-card">
                <h4 className="rp-chart-title">Análisis de Residuos (Multidimensional)</h4>
                <ResponsiveContainer width="100%" height={230}>
                  <RadarChart cx="50%" cy="50%" outerRadius="75%" data={networkData.slice(0, 5)}>
                    <PolarGrid stroke="rgba(255,255,255,0.1)" />
                    <PolarAngleAxis dataKey="name" tick={{ fill: '#94a3b8', fontSize: 11 }} />
                    <PolarRadiusAxis angle={30} domain={[0, 'auto']} tick={false} axisLine={false} />
                    <Tooltip content={<ChartTooltip />} />
                    <Radar name="Menciones" dataKey="count" stroke="#0ea5e9" fill="#0ea5e9" fillOpacity={0.4} filter="url(#glow)" />
                  </RadarChart>
                </ResponsiveContainer>
              </div>
                    ...
                    {networkData.map(n => {
                      ...
                      return (
                        <Area key={netKey} type="monotone" dataKey={netKey}
                          name={n.name} stroke={n.color} activeDot={{ r: 6, strokeWidth: 0, fill: '#fff', filter: 'url(#glow)' }}
                          fill={`url(#reach-${netKey})`} strokeWidth={2} stackId="1" />
                      );
                    })}
                  </AreaChart>
                </ResponsiveContainer>
            ...
            <div className="rp-chart-card rp-chart-card--full">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <AreaChart data={timelineData} margin={{ top: 4, right: 16, left: -20, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey="total"    name="Total"    stroke="#f093fb" fill="url(#gradTotal)" strokeWidth={3} activeDot={{ r: 6, strokeWidth: 0, fill: '#fff', filter: 'url(#glow)' }} />
                  <Area type="monotone" dataKey="positive" name="Positivo" stroke="#10b981" fill="url(#gradPos)"   strokeWidth={3} activeDot={{ r: 6, strokeWidth: 0, fill: '#fff', filter: 'url(#glow)' }} />
                  <Area type="monotone" dataKey="negative" name="Negativo" stroke="#ef4444" fill="url(#gradNeg)"   strokeWidth={3} activeDot={{ r: 6, strokeWidth: 0, fill: '#fff', filter: 'url(#glow)' }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <PieChart>
                    <Pie data={sentimentData} cx="50%" cy="50%" innerRadius={60} outerRadius={90}
                      dataKey="value" paddingAngle={4} stroke="#ffffff" strokeWidth={2}
                ...
                <ResponsiveContainer width="100%" height={230}>
                  <BarChart data={networkData} layout="vertical" margin={{ top: 4, right: 20, left: 70, bottom: 0 }}>
                    ...
                    <Bar dataKey="count" name="Menciones" radius={[0, 4, 4, 0]}>
                      {networkData.map((n, i) => <Cell key={i} fill={`url(#bar-grad-${i % CHART_COLORS.length})`} stroke={n.color} strokeWidth={1} filter="url(#glow)" />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
