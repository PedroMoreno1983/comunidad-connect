...
// --- Honestidad de la UI (reescrita para el cargador 1.2.0) ---
// Los checks anteriores apuntaban a la UI de 0.3.11, que fue reemplazada por
// completo. Se conserva la INTENCION -no prometer un carro que no existe- pero
// verificando el comportamiento de la UI actual, no sus strings.
check(button.includes('<CartLoaderButton') || page.includes('<CartLoaderButton'), 'La pagina no monta el cargador.');
check(button.includes('basket={basket}') || page.includes('basket={selectedBasket}'), 'El cargador no recibe la canasta elegida.');
// Sin extension NO puede anunciarse un carro cargado.
check(
  button.includes('!hasExtension') && button.includes('Falta el Cargador de Convive'),
  'Sin extension la UI podria seguir prometiendo una carga automatica.',
);
// El limite de seguridad vale siempre: CoCo agrega, la persona confirma y paga.
check(
  button.includes('confirmas la entrega y pagas'),
  'La UI perdió el límite de seguridad y pago manual.',
);
// Irurzun es cotizacion mayorista, no un carro con precio final.
check(
  button.includes("basket.store === 'Irurzun'") && button.includes('mayorista') && button.includes('cotización'),
...
check(
  button.includes('useSupermarketCartLoader')
