...
const extensionPath = path.resolve(__dirname, '..', 'extensions', 'convive-shopping-assistant');
const profilePath = fs.mkdtempSync(path.join(os.tmpdir(), 'convive-shopping-assistant-'));
const stores = [
  {
    name: 'Lider',
...
async function main() {
  ...
  try {
    ...
    if (
      !extensionIdentity
      || extensionIdentity.version !== '2.0.0'
      || !extensionIdentity.capabilities?.includes('shopping-assistant-v1')
