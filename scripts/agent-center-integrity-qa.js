...
const whatsappStatusRoute = read('src/app/api/whatsapp/status/route.ts');
const whatsappStatusCallback = read('src/app/api/whatsapp/status-callback/route.ts');
const twilioContentTemplate = read('src/lib/server/twilioContentTemplate.ts');
...
const checks = [
  ...
  ['Ambiguous payment reminders request a private recipient before acting', route.includes('wantsPaymentReminderIntent') && route.includes('Indica el departamento o el nombre del residente') && route.indexOf('if (wantsPaymentReminderIntent)') < route.indexOf("if (lower.includes('comunicado')")],
  ['Announcement title cleanup removes command wording without orphaned articles', route.includes('env[ií]ale') && route.includes('(?:un|una|el|la)?')],
