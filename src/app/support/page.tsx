...
export const metadata = {
  title: "Soporte",
  description: "Canales de soporte y activación para administradores, residentes y conserjería de Convive Connect.",
...
const supportLanes = [
  {
    ...
    response: "Prioridad alta en horario hábil.",
  },
  {
    title: "Residentes",
    detail: "Acceso, pagos, reservas, notificaciones, WhatsApp y convivencia vecinal.",
    response: "Respuesta según SLA de comunidad.",
  },
  {
    title: "Conserjería",
    detail: "Visitas, paquetes, avisos urgentes, bitácora y derivación de casos CoCo.",
        ...
        <section className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-base leading-8 text-[#524A40]">
              Centralizamos soporte comercial, técnico y operativo para que cada edificio tenga una ruta clara:
          ...
          <div className="rounded-2xl border border-[var(--cc-line-strong)] bg-white/70 p-6 shadow-sm">
            ...
            <div className="mt-6 space-y-3 text-sm text-[#524A40]">
              <p className="flex gap-2"><Clock className="mt-0.5 h-4 w-4 text-[#733D24]" /> Recepción y triage por prioridad operacional.</p>
              <p className="flex gap-2"><Shield className="mt-0.5 h-4 w-4 text-[#5F7A46]" /> Tratamiento de datos con finalidad, minimización y acceso por rol.</p>
              <p className="flex gap-2"><Smartphone className="mt-0.5 h-4 w-4 text-[#5C4868]" /> Activación guiada de CoCo IA por WhatsApp.</p>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-[#D9CABA] bg-[#111827] p-6 text-white shadow-sm md:p-8">
          <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-semibold">Activación de CoCo IA</h2>
              <p className="mt-2 max-w-2xl text-sm leading-7 text-white/70">
                Escribe a {SUPPORT_EMAIL} para activar CoCo por WhatsApp en tu comunidad. El equipo configura el canal internamente; no publiques ni reenvíes endpoints de integración.
              </p>
            </div>
            <a className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-semibold text-white/90" href={`mailto:${SUPPORT_EMAIL}`}>
              {SUPPORT_EMAIL}
            </a>
          </div>
          <div className="mt-6 flex flex-wrap gap-3">
            <a href={`mailto:${SUPPORT_EMAIL}?subject=Activar WhatsApp Convive`} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-bold text-[#111827]">
              Solicitar activación <MessageCircle className="h-4 w-4" />
            </a>
