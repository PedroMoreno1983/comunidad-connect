        ...
        <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6">
            <header className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <button onClick={() => setShowTask(true)} className="inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors" style={{ background: "var(--cc-copper)" }}>
                        <Plus className="h-4 w-4" />
                        Nueva tarea
                    </button>
                    <button onClick={exportCsv} className="inline-flex items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-semibold cc-text-primary transition-colors hover:bg-[var(--cc-paper-warm)]" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }}>
            ...
            <section className="rounded-2xl border" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }}>
                <div className="flex flex-col gap-4 border-b p-5 md:flex-row md:items-center md:justify-between" style={{ borderColor: "var(--cc-line)" }}>
                    <div className="flex items-center gap-3">
                        <span className="flex h-10 w-10 items-center justify-center rounded-full" style={{ background: "var(--cc-copper-tint)", color: "var(--cc-copper)" }}>
                            <Gauge className="h-5 w-5" />
                        </span>
                        <div>
                            <h2 className="font-semibold cc-text-primary" style={{ fontFamily: "var(--cc-font-display)", fontSize: 19 }}>Pulso del edificio</h2>
                            <p className="text-sm font-medium cc-text-secondary">Salud global de activos y cola operativa.</p>
                        </div>
                    </div>
                    <div className="min-w-[220px]">
                        ...
                        <div className="h-2 overflow-hidden rounded-full" style={{ background: "var(--cc-paper-warm)" }}>
                            <div className="h-full rounded-full" style={{ width: `${metrics.healthScore}%`, background: "var(--cc-copper)" }} />
                        </div>
                    </div>
                </div>
                ...
                <Metric icon={<Bot className="h-5 w-5" />} label="Casos CoCo activos" value={metrics.activeCases} tone="sage" detail={`${metrics.criticalCases} alta prioridad`} />
                <Metric icon={<Wrench className="h-5 w-5" />} label="Solicitudes cerradas" value={`${metrics.completed}/${metrics.total}`} tone="copper" detail="Ciclo mensual" />
                ...
                <Metric icon={<Activity className="h-5 w-5" />} label="Costo mantención" value={money(metrics.cost)} tone="ink" detail="Últimos registros" />
                </div>
            </section>
            ...
            <nav className="flex gap-2 overflow-x-auto border-b" style={{ borderColor: "var(--cc-line)" }}>
                {[
                    ["operacion", "Operación"],
                    ["activos", "Activos"],
                    ["sensores", "Sensores"],
                    ...
                    <button
                        ...
                        style={activeTab === key
                            ? { borderColor: "var(--cc-copper)", color: "var(--cc-copper)" }
                            : { borderColor: "transparent", color: "var(--cc-ink-muted)" }}
                    >
                        {label}
                    </button>
                ))}
            ...
            {loading ? (
                <div className="flex h-80 items-center justify-center rounded-2xl border" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }}>
                    <Loader2 className="h-7 w-7 animate-spin" style={{ color: "var(--cc-copper)" }} />
                </div>
            ) : activeTab === "operacion" ? (
                                ...
                                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                                    <div className="min-w-0 space-y-2">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            <Badge tone={item.status === "completed" ? "sage" : "copper"}>{statusLabel(item.status)}</Badge>
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                    {assets.map(asset => {
                        ...
                        const healthStyle = health === "critical"
                        ...
                        return (
                            <article key={asset.id} className="rounded-2xl border p-6" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }}>
                                <div className="mb-6 flex items-start justify-between gap-4">
                                    <div className="flex h-12 w-12 items-center justify-center rounded-full" style={healthStyle}>
                                        {asset.category === "pump" ? <Droplets className="h-5 w-5" /> : asset.category === "electrical" ? <Zap className="h-5 w-5" /> : <Wrench className="h-5 w-5" />}
                                    </div>
                                    <Badge tone={health === "critical" ? "rose" : health === "warning" ? "amber" : "sage"}>{health}</Badge>
                                </div>
                                <h3 className="text-lg font-semibold cc-text-primary" style={{ fontFamily: "var(--cc-font-display)" }}>{asset.name || "Activo tecnico"}</h3>
                ...
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                    {[
                        ...
                        <article key={String(id)} className="rounded-2xl border p-6" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }}>
                            <div className="mb-5 flex items-center justify-between">
                                <span className="flex h-11 w-11 items-center justify-center rounded-full" style={{ background: "var(--cc-sage-tint)", color: "var(--cc-sage)" }}>{icon}</span>
                                <span className="h-2.5 w-2.5 rounded-full" style={{ background: "var(--cc-sage)", boxShadow: "0 0 12px color-mix(in srgb, var(--cc-sage) 60%, transparent)" }} />
                            </div>
                            <h3 className="font-semibold cc-text-primary" style={{ fontFamily: "var(--cc-font-display)" }}>{title}</h3>
                            ...
                            <div className="mt-5 flex items-center justify-between text-xs font-bold">
                                ...
                                <span style={{ color: "var(--cc-sage)" }}>{status}</span>
                            </div>
                        </article>
                    ))}
                </div>
            ...
            {showTask && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={() => setShowTask(false)}>
                    <div className="w-full max-w-lg rounded-2xl border p-6" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper)" }} onClick={event => event.stopPropagation()}>
                        <div className="mb-6 flex items-center justify-between">
                            <h2 className="text-xl font-semibold cc-text-primary" style={{ fontFamily: "var(--cc-font-display)" }}>Nueva tarea</h2>
                            <button onClick={() => setShowTask(false)} className="rounded-full p-2 hover:bg-[var(--cc-paper-warm)]"><X className="h-5 w-5" /></button>
                        </div>
                        <div className="space-y-4">
                            <input value={form.title} onChange={event => setForm(prev => ({ ...prev, title: event.target.value }))} className="w-full rounded-xl border px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--cc-copper)] focus:ring-2 focus:ring-[var(--cc-copper)]/15" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper-warm)" }} placeholder="Título" />
                            <textarea value={form.description} onChange={event => setForm(prev => ({ ...prev, description: event.target.value }))} className="h-28 w-full resize-none rounded-xl border px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--cc-copper)] focus:ring-2 focus:ring-[var(--cc-copper)]/15" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper-warm)" }} placeholder="Descripción del trabajo" />
                            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                <select value={form.service_type} onChange={event => setForm(prev => ({ ...prev, service_type: event.target.value }))} className="rounded-xl border px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--cc-copper)]" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper-warm)" }}>
                                    {categories.map(category => <option key={category} value={category}>{category}</option>)}
                                </select>
                                <input type="date" value={form.scheduled_date} onChange={event => setForm(prev => ({ ...prev, scheduled_date: event.target.value }))} className="rounded-xl border px-4 py-3 text-sm font-semibold outline-none focus:border-[var(--cc-copper)]" style={{ borderColor: "var(--cc-line)", background: "var(--cc-paper-warm)" }} />
                            </div>
                            <button onClick={saveTask} disabled={saving} className="flex h-12 w-full items-center justify-center gap-2 rounded-full text-sm font-semibold text-white disabled:opacity-50" style={{ background: "var(--cc-ink)" }}>
...
function Metric({ icon, label, value, detail, tone }: { icon: React.ReactNode; label: string; value: React.ReactNode; detail: string; tone: "sage" | "copper" | "amber" | "ink" }) {
    const colors = {
        sage: { background: "var(--cc-sage-tint)", color: "var(--cc-sage)" },
        copper: { background: "var(--cc-copper-tint)", color: "var(--cc-copper)" },
        amber: { background: "var(--cc-amber-tint)", color: "var(--cc-amber)" },
        ink: { background: "var(--cc-paper-warm)", color: "var(--cc-ink)" },
    };

    return (
        <article className="p-5">
            <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-full" style={colors[tone]}>{icon}</div>
...
function Badge({ children, tone = "ink" }: { children: React.ReactNode; tone?: "ink" | "copper" | "sage" | "rose" | "amber" }) {
    const colors = {
        ink: { background: "var(--cc-paper-warm)", color: "var(--cc-ink-muted)" },
        copper: { background: "var(--cc-copper-tint)", color: "var(--cc-copper)" },
        sage: { background: "var(--cc-sage-tint)", color: "var(--cc-sage)" },
    ...
    return <span className="rounded-full px-2 py-1 text-[10px] font-bold uppercase tracking-[0.14em]" style={colors[tone]}>{children}</span>;
}

function SectionTitle({ icon, title }: { icon: React.ReactNode; title: string }) {
    return (
        <div className="flex items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-full cc-text-primary" style={{ background: "var(--cc-paper-warm)" }}>{icon}</span>
            <h2 className="text-lg font-semibold cc-text-primary" style={{ fontFamily: "var(--cc-font-display)" }}>{title}</h2>
            <ArrowRight className="h-4 w-4" style={{ color: "var(--cc-ink-faint)" }} />
        </div>
    );
}
