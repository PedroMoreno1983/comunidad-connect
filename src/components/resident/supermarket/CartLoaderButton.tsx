...
export function CartLoaderButton({ basket, onQuote, onSelect }: CartLoaderButtonProps) {
  ...
  const loaderFailed = loaderProgress?.status === 'failed';
  const loaderCompletedWithIssues = loaderProgress?.status === 'completed_with_issues';
  const loaderCompleted = loaderProgress?.status === 'completed';
      ...
      {loaderProgress && (
        ...
        >
          <div className="flex items-center gap-2 text-xs font-semibold cc-text-primary">
            ...
            <span>
              {loaderFailed
                ? `No se pudo confirmar el carro de ${basket.store}`
                : loaderCompletedWithIssues
                  ? `Carga incompleta en ${basket.store}`
                  : loaderCompleted
                    ? `Carro de ${basket.store} confirmado`
