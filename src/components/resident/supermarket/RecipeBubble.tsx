    ...
    return (
        <div className="w-full max-w-sm bg-white dark:bg-slate-800 rounded-lg overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700">
            ...
            <div className="p-4 border-b border-slate-100 dark:border-slate-700 space-y-2">
                <h4 className="font-black text-slate-900 dark:text-white text-lg">{recipe.name}</h4>
                ...
                {missingIngredients.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between text-sm">
                        ...
                        {item.marketOffer ? (
                            ...
                            <span className="flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded-full">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-4 space-y-3">
                ...
                {marketplaceMissing.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-white dark:bg-slate-800 p-3 rounded-xl border border-blue-100 dark:border-blue-900">
                        <div className="text-xs">
                            <p className="font-bold text-slate-900 dark:text-white">{item.marketOffer?.itemTitle}</p>
