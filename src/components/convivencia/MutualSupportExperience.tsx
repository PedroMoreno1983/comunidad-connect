...
import type {
  SolidarityActiveTab,
  SolidarityApplication,
  SolidarityFund,
  SolidarityLedgerEntry,
  SolidarityTask,
} from "@/lib/types";
...
export function MutualSupportExperience() {
  const { user } = useAuth();
  const { toast } = useToast();

  // Tabs: 'transparency' | 'apply' | 'tasks'
  const [activeTab, setActiveTab] = useState<SolidarityActiveTab>("transparency");

  // Data loading state
  const [loading, setLoading] = useState(true);
  const [fund, setFund] = useState<SolidarityFund | null>(null);
  const [ledger, setLedger] = useState<SolidarityLedgerEntry[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [fundData, appData, tasksData] = await Promise.all([
        fetchJsonWithTimeout<{ fund: SolidarityFund; ledger: SolidarityLedgerEntry[] }>("/api/solidarity/fund"),
